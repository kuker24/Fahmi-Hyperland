...
BRANCH="${1:-master}"
REPO_DIR="$HOME"
MAINTENANCE_DIR="$HOME/.config/fahmi/hypr/maintenance"
...
"$HOME/.config/fahmi/hypr/scripts/bar.sh &"
...
if [[ -n "$aur_helper" ]]; then
    ...
    "$HOME/.config/fahmi/hypr/pacman/install-pkgs.sh $aur_helper" \
    "y"
else
...
run_interactive_step "🔌" \
"Updating plugins" \
"$HOME/.config/fahmi/hypr/maintenance/PLUGINS.sh" \
"y"
