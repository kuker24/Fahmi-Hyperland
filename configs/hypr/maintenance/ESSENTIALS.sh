...
install_browser() {
    ...
    echo -e "exec-once = $app \n windowrule = workspace 2 silent, match:title ^($title)$" >$HOME/.config/fahmi/hypr/configs/defaults/browser.conf
...
install_discord_client() {
    ...
    echo -e "workspace = 6, gapsout:69, on-created-empty:$class \n windowrule = workspace 6 silent, match:class ^.*cord$" >$HOME/.config/fahmi/hypr/configs/defaults/discord_client.conf
}
